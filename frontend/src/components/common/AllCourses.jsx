...
const AllCourses = () => {
   ...
   const handleSubmit = async (courseId) => {
      try {
          ...
          const formattedData = {
              cardDetails,
              S_title: selectedCourse.sections.map(section => section.S_title),
              S_description: selectedCourse.sections.map(section => section.S_description),
          };
  
          const res = await axiosInstance.post(`api/user/enrolledcourse/${courseId}`, formattedData, {
              headers: {
                  Authorization: `Bearer ${localStorage.getItem('token')}`,
              },
          });
